...
**9 тулов.** Календарь: `calendar_schedule`, `calendar_event`, `calendar_respond`,
`calendar_cancel`. Парковка и офис: `parking_places`, `parking_book`,
`office_bookings`. Сессия: `myt_status`, `myt_refresh_session`.
...
переговорку, отменить бронь парковки. Соответствующих запросов у клиента не
наблюдалось, а собирать их наугад на живом рабочем сервисе — это разослать мусор
коллегам или снять чужую бронь.
...
# MCP-сервер:
claude mcp add myt -- ./.venv/bin/python -m src.server
...
Полный вход нужен только когда тул сказал `MYT SESSION EXPIRED`.
...
| `MYT_TRACE_FILE`, `MYT_EVENTS` | Куда писать трассировку и события |
...
- Пароль не проходит через агента: только `login_cli.py` и `getpass`.
