...
def main():
    ...
    tmp = tempfile.mkdtemp(prefix="myt-tests-")
    env = dict(os.environ)
    # Прогон тестов не должен дописывать ничего в реальные файлы пользователя.
    env["MYT_SESSION"] = os.path.join(tmp, "session.json")
    env["MYT_EVENTS"] = os.path.join(tmp, "events.jsonl")
    env["MYT_TRACE_FILE"] = os.path.join(tmp, "calls.jsonl")
