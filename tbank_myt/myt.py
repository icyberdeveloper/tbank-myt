...
class MytSession:
    ...
    def __post_init__(self) -> None:
        # Ставится владельцем (server). Наблюдаемый сервер refresh-токен НЕ ротирует
        # (обмен возвращает тот же самый), но код принимает любой присланный, и если
        # сервер однажды начнёт ротировать, не записанная на диск ротация сожжёт
        # токен для следующего процесса — поэтому сохранение обязательно.
        ...
        # Бандл = системные CA + приколотый корень Минцифры. Нужен он ради ОДНОГО
        # хоста — magentbep.tcsbank.ru, который выдаёт и обменивает токен: его цепочка
        # идёт через Russian Trusted Root CA, которого нет ни в одном системном сторе.
        # kairos и workplacer проходят и на голых системных CA — подробности в tls.py.
        try:
