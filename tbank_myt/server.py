...
def parking_places(date: str = "", building_id: int = 0) -> str:
    ...
    try:
        ...
        head = [
            ...
            # Два числа связаны, а не независимы: глубина окна — это глубина ПОСЛЕ
            # открытия. Измерено — в 23:48 день D+3 бронировался, а в 00:32, когда
            # он стал D+2, сервер ответил «ещё не открыто, приходите в 12:00».
            # Насколько окно короче до полудня, из наблюдений не выведено, поэтому
            # число не называется: сказать «доступны только сегодня и завтра» значило
            # бы выдать одну замеренную границу за правило.
            f"Бронь открыта на {horizon} дн. вперёд — это глубина ПОСЛЕ открытия в "
            f"{cfg.get('openParkingAccessTime')} (время сервера). До открытия дальние "
            f"дни не бронируются, даже когда места по ним показаны свободными.",
