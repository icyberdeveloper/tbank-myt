"""CA trust for the MyT MCP.

Три хоста, и доверие им нужно разное — проверено запросом, а не выведено из
суффикса имени:

  * `kairos.tbank.ru` и `workplacer.tbank.ru` проходят на системных CA. Календарь
    и парковка работали бы и без этого файла.
  * `magentbep.tcsbank.ru` — НЕТ. Он отдаёт цепочку с **Russian Trusted Root CA**
    (Минцифры), которого не везёт ни один системный стор, и на голых системных CA
    падает с CERTIFICATE_VERIFY_FAILED.

Второй пункт и есть причина, по которой файл здесь. Это хост выдачи и обмена
токена: без него не проходит ни логин, ни продление, то есть не работает ничего,
хотя два «рабочих» хоста и выглядят доступными.

Бандл = системные CA + приколотый русский корень, и больше ничего.
...
root ever IS replaced, drop the new PEM into `ca/roots/` or point `MYT_EXTRA_CA`
...
# Каждый хост, с которым говорит этот MCP. В банковском репозитории список
# выводился из таблицы эндпоинтов, потому что там их два десятка и рукописный
# перечень отставал. Здесь их три, все названы в модуле myt, и выводить не из
# чего — зато добавление четвёртого обязано попасть сюда же, иначе он окажется
# вне проверки полноты хранилища доверия.
BANK_HOSTS = [
    "kairos.tbank.ru",          # календарь
    "magentbep.tcsbank.ru",     # выдача и обмен токена
    "workplacer.tbank.ru",      # места и парковка
]
...
def _log(msg: str) -> None:
    print(f"[myt-tls] {msg}", file=sys.stderr, flush=True)
...
def load_roots(roots_dir: str | None = None) -> list[str]:
    ...
    extra = os.environ.get("MYT_EXTRA_CA", "")
    for path in [p for p in extra.split(os.pathsep) if p]:
        try:
            ...
            _log(f"MYT_EXTRA_CA {path}: {e}")
            continue
        _log(f"trusting {len(got)} certificate(s) from MYT_EXTRA_CA {path} "
        ...
        try:
            ...
            try:
                ...
                raise requests.exceptions.SSLError(
                    f"{e}\n[myt-tls] Certificate verification failed against the "
                    ...
                    f"PEM to that directory (or set MYT_EXTRA_CA) and retry; "
